...
d = {1: 'a', 2: 'b', 3: 'd'}
d.keys() - выведет все ключи dict_keys([1, 2, 3]) -> list(d)
d.values() - выведет все значения dict_values(['a', 'b', 'd']) -> list(d)
d.items() - dict_items([(1, 'a'),( 2, 'b'), (3, 'd')]) -> list(d)
1: 495,
2: 812,
3: 8412
d.get() # безопасно забрать значение по ключу
...
d.setdefault() # безопасно запишет в словарь (ключ: значение) (если такой ключ есть, то перезаписывать не будет)
