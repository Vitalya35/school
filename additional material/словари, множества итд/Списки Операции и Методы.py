...
a.remove() - удаляет элемент по значению, первый попавшийся подходящий элемент a[0] - удаляет первый элемент под индексом 0
del numbers[::2] - удалить все элементы на четных позициях (0, 2, 4, ...) исходного списка.
a.pop() или a.pop(3)- удаляет элемент по индекску и выводит на экран, по умолчанию последний

Сортировка:
sorted() - сортировка, изначальный список не меняется
sorted(a,reverse=True) - сортировка по убыванию
a.sort() - сортирует список по возрастанию, изменяя изначальный список (нельзя присваивать переменной и выводить в print)
...
Пример:
a = [('ivanov', 2002)('Petrov', 2004)('Sidorov', 1980)] # dict(a)
...
b=[int(symbol) for symbol in s if symbol.isdigit()] - создаст список из цифр из переменной s
