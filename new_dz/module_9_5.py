...
# первую очередь проверяется step на равенство 0. Если равно, то выбрасывается исключение StepValueError('шаг не может быть равен 0')
...
class Iterator:
    ...
    def __init__(self, start, stop, step=1):
        ...
        if not isinstance(self.start, int):
            raise print('значения долны быть целыми числами')
        self.stop = stop
    ...
    def __iter__(self):
        self.pointer, self.start = self.pointer, self.start
        return self
